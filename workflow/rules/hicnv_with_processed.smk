...
rule download_from_4dn_nucleome: # (status: running)
    shell:
        r"""
            curl -L \
...
# helper function for rule download_4d_nucleome_alns
def get_4dn_bam_url(wildcards):
    # loading 4dn meta data
    meta = read_table('config/fastq_4dn_meta.tsv')
...
# Download 4D Nucleome bam data
use rule download_from_4dn_nucleome as download_4d_nucleome_alns with: # (status: running)
    params:
        key = 'GBYDTYDO',
        secret = 'kkrklx5k5n7v7bci',
        url = get_4dn_bam_url 
    output:
        'results/main/{cline}/4d_nucleome/tech_reps/{srr}/{srr}.bam'
    log:
        'results/main/{cline}/logs/rule_download_4d_nucleome_alns_{cline}_{srr}.log'


# helper function for rule download_4d_nucleome_alns
def get_4dn_pairs_url(wildcards): # (status: running)
    # loading 4dn meta data
    print('# loading 4dn meta data')
    meta = read_table('config/fastq_4dn_meta.tsv')
    meta = meta[meta.file_type == 'pairs']
    indexes = []
    for i, sr in meta.iterrows():
        idx = '{}-B{}-T{}'.format(sr.exp_acc, sr.biorep, sr.techrep)
        indexes.append(idx)
    ...
    # getting the file accession for the current data
    file_acc = meta.loc[wildcards.srr, 'file_acc']
    url = 'https://data.4dnucleome.org/files-processed/{srr}/@@download/{srr}.pairs.gz'
    url = url.format(srr=file_acc)
    return(url)

# Download 4D Nucleome bam data
use rule download_from_4dn_nucleome as download_4d_nucleome_pairs with: # (status: running)
    params:
        key = 'GBYDTYDO',
        secret = 'kkrklx5k5n7v7bci',
        url = get_4dn_pairs_url 
    output:
        protected('results/main/{cline}/4d_nucleome/tech_reps/{srr}/{srr}.pairs.gz')
    log:
        'results/main/{cline}/logs/rule_download_4d_nucleome_pairs_{cline}_{srr}.log'


# helper function for rule download_4d_nucleome_fastq
def get_4dn_fastq_url(wildcards): # (status: developing)
    """
    Returns the URL for both R1 and R2. 
    Uses the experiment name as the SRR-like ID and assigns
    R1 and R2 accordign to the config/4dn_meta.tsv.
    """

    # loading 4dn meta data
    print('# loading 4dn meta data')
    meta = read_table('config/fastq_4dn_meta.tsv')
    meta = meta[(meta.file_type == 'fastq') & (meta.r1_or_r2 == wildcards.rnum)]
    indexes = []
    for i, sr in meta.iterrows():
        idx = '{}-B{}-T{}'.format(sr.exp_acc, sr.biorep, sr.techrep)
        indexes.append(idx)
    ...
    # getting the file accession for the current data
    file_acc = meta.loc[wildcards.srr, 'file_acc']
    url = 'https://data.4dnucleome.org/files-processed/{srr}/@@download/{srr}.fastq.gz'
    url = url.format(srr=file_acc)
    return(url)

# Download 4D Nucleome fastq data
use rule download_from_4dn_nucleome as download_4d_nucleome_fastq with: # (status: running)
    params:
        key = 'GBYDTYDO',
        secret = 'kkrklx5k5n7v7bci',
        url = get_4dn_fastq_url 
    output:
        protected('results/main/{cline}/reads/{srr}_{rnum}.fastq.gz')
    log:
        'results/main/{cline}/logs/rule_download_4d_nucleome_fastq_{cline}_{srr}_{rnum}.log'
